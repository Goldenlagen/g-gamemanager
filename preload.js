...
contextBridge.exposeInMainWorld('api', {
  ...
  sgdbStatus: () => ipcRenderer.invoke('sgdb-status'),
  sgdbFetch: (gameKey, gameName) => ipcRenderer.invoke('sgdb-fetch', gameKey, gameName),
  sgdbClear: (gameKey) => ipcRenderer.invoke('sgdb-clear', gameKey),
  ...
  mcDeleteServer: (serverPath) => ipcRenderer.invoke('mc-delete-server', serverPath),
  mcShareInfo: (serverPath) => ipcRenderer.invoke('mc-share-info', serverPath),
  mcListPlayers: (serverPath) => ipcRenderer.invoke('mc-list-players', serverPath),
  mcBackupList: (serverPath) => ipcRenderer.invoke('mc-backup-list', serverPath),
  mcBackupCreate: (serverPath) => ipcRenderer.invoke('mc-backup-create', serverPath),
  mcBackupRestore: (serverPath, name) => ipcRenderer.invoke('mc-backup-restore', serverPath, name),
  mcBackupDelete: (serverPath, name) => ipcRenderer.invoke('mc-backup-delete', serverPath, name),
  mcAccessList: (serverPath) => ipcRenderer.invoke('mc-access-list', serverPath),
  mcAccessAdd: (serverPath, list, name) => ipcRenderer.invoke('mc-access-add', serverPath, list, name),
  mcAccessRemove: (serverPath, list, name) => ipcRenderer.invoke('mc-access-remove', serverPath, list, name),
  ...
  cfGetModLogo: (modId) => ipcRenderer.invoke('cf-get-mod-logo', modId),
  ...
  mcpQuickLaunchProfile: (profileKey) => ipcRenderer.invoke('mcp-quick-launch-profile', profileKey),
  ...
  arkRcon: (serverPath, command) => ipcRenderer.invoke('ark-rcon', serverPath, command),
  arkDeleteServer: (serverPath) => ipcRenderer.invoke('ark-delete-server', serverPath),
  ...
  dashboardList: () => ipcRenderer.invoke('dashboard-list'),
  dashboardStats: () => ipcRenderer.invoke('dashboard-stats'),
  dashboardStart: (server) => ipcRenderer.invoke('dashboard-start', server),
  dashboardStop: (server) => ipcRenderer.invoke('dashboard-stop', server),
  dashboardForceStop: (server) => ipcRenderer.invoke('dashboard-force-stop', server),
  dashboardSetAutostart: (id, enabled) => ipcRenderer.invoke('dashboard-set-autostart', id, enabled),
  dashboardSetAutorestart: (id, enabled) => ipcRenderer.invoke('dashboard-set-autorestart', id, enabled),
  dashboardSetRestartInterval: (id, hours) => ipcRenderer.invoke('dashboard-set-restart-interval', id, hours),
  ...
  // Langue : synchronise la langue courante avec le processus principal
  // (libellés natifs + messages d'erreur backend).
